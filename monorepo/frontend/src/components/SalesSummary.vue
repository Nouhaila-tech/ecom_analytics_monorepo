<template>
  ...
  <div class="flex flex-col col-span-full sm:col-span-6 xl:col-span-4 bg-white dark:bg-gray-800 shadow-sm rounded-xl">
    ...
    <div class="grow max-sm:max-h-[128px] xl:max-h-[128px]">
      <LineChart :data="monthlySalesData" width="389" height="128" />
    </div>
  </div>
</template>
...
export default defineComponent({
  ...
  setup() {
    ...
    const monthlySalesData = ref<ChartData<'line'> | null>(null);

    const defaultStartDate ='2024-01-01';
    const defaultEndDate ='2024-07-30';

    onMounted(async () => {
      await fetchSalesData(defaultStartDate, defaultEndDate);
    });
